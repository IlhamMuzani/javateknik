...
@section('content')
    ...
    <script>
        ...
        function handleScan() {
            ...
            if (scanValue) {
                addPesanan(); // Tambah baris baru
                currentRowIndex = jumlah_ban; // Simpan index baris terakhir yang ditambahkan

                $('#searchInput').val(scanValue); // Isi kolom pencarian
                ...
                setTimeout(function() {
                    ...
                    if (visibleRows.length === 1) {
                        visibleRows.find('button').trigger('click'); // Klik otomatis jika hanya satu hasil
        ...
        function getBarang(index) {
            ...
            if (currentRowIndex !== null) {
                ...
                $('#harga-' + currentRowIndex).val(harga);
                $('#jumlah-' + currentRowIndex).val(1);

                var total = harga * 1;
                $('#total-' + currentRowIndex).val(total);

                grandTotal(); // Update total keseluruhan

                closeTableBarang(); // Menyembunyikan tableBarang
            }

            currentRowIndex = null; // Reset index
        }
