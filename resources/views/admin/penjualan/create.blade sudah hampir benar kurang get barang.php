    ...
    <script>
        ...
        function handleScan() {
            ...
            if (scanValue) {
                ...
                // Menampilkan modal pencarian barang
                $('#tableBarang').modal('show');
        ...
        // Fungsi untuk mengambil barang yang dipilih
        function getBarang(index) {
            var barang = $('#tables tr').eq(index + 1); // Mengambil data dari baris yang dipilih
            var barang_id = barang.data('barang_id');
            var kode_barang = barang.data('kode_barang');
            var nama_barang = barang.data('nama_barang');
            var harga = barang.data('harga');

            // Update form dengan data barang yang dipilih
            $('#kode_barang-' + (jumlah_ban - 1)).val(kode_barang);
            $('#nama_barang-' + (jumlah_ban - 1)).val(nama_barang);
            $('#harga-' + (jumlah_ban - 1)).val(harga);
