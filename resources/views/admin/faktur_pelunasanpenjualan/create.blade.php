...
@section('content')
    ...
    <section class="content" style="display: none;" id="mainContentSection">
        <div class="container-fluid">
            ...
            @endif
            <form action="{{ url('admin/pelunasan-penjualan') }}" method="POST" enctype="multipart/form-data"
                autocomplete="off">
                @csrf
                <div class="card">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody id="tabel-penjualan">
                                <tr id="penjualan-0">
                                    ...
                                    <td hidden>
                                        <div class="form-group">
                                            <input type="text" class="form-control" id="penjualan_id-0"
                                                name="penjualan_id[]">
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-group">
                                            <input onclick="MemoEkspedisi(0)" style="font-size:14px" readonly
                                                type="text" class="form-control" id="kode_penjualan-0"
                                                name="kode_penjualan[]">
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-group">
                                            <input onclick="MemoEkspedisi(0)" style="font-size:14px" readonly
                                                type="text" class="form-control" id="tanggal_penjualan-0"
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6" style="margin-left: 89px">
                                ...
                                <div class="form-group">
                                    <label style="font-size: 14px" for="tinggi">Potongan Penjualan</label>
                                    <input style="text-align: end; font-size: 14px" type="text" class="form-control"
        ...
        <div class="modal fade" id="tablePenjualan" data-backdrop="static">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Data Faktur Penjualan</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
    ...
    <script>
        ...
        function addPesanan() {
            ...
            itemPenjualan(jumlah_ban, jumlah_ban - 1);
        }

        function removePenjualan(params) {
            jumlah_ban = jumlah_ban - 1;

            var tabel_pesanan = document.getElementById('tabel-penjualan');
            var penjualan = document.getElementById('penjualan-' + params);

            tabel_pesanan.removeChild(penjualan);

            if (jumlah_ban === 0) {
                var item_penjualan = '<tr>';
                item_penjualan += '<td class="text-center" colspan="5">- Memo belum ditambahkan -</td>';
                item_penjualan += '</tr>';
                $('#tabel-penjualan').html(item_penjualan);
            } else {
        ...
        function itemPenjualan(urutan, key, value = null) {
            var penjualan_id = '';
            var kode_penjualan = '';
            ...
            // urutan 
            var item_penjualan = '<tr id="penjualan-' + urutan + '">';
            item_penjualan += '<td style="width: 70px; font-size:14px" class="text-center" id="urutan-' + urutan + '">' +
                urutan + '</td>';

            // penjualan_id 
            item_penjualan += '<td hidden>';
            item_penjualan += '<div class="form-group">'
            item_penjualan += '<input type="text" class="form-control" id="penjualan_id-' + urutan +
                '" name="penjualan_id[]" value="' + penjualan_id + '" ';
            item_penjualan += '</div>';
            item_penjualan += '</td>';

            // kode_penjualan
            item_penjualan += '<td onclick="MemoEkspedisi(' + urutan +
                ')">';
            item_penjualan += '<div class="form-group">'
            item_penjualan +=
                '<input type="text" class="form-control" style="font-size:14px" readonly id="kode_penjualan-' +
                urutan +
                '" name="kode_penjualan[]" value="' + kode_penjualan + '" ';
            item_penjualan += '</div>';
            item_penjualan += '</td>';
            ...
            item_penjualan += '<div class="form-group">'
            item_penjualan +=
                '<input type="text" class="form-control" style="font-size:14px" readonly id="tanggal_penjualan-' +
                urutan +
                '" name="tanggal_penjualan[]" value="' + tanggal_penjualan + '" ';
            item_penjualan += '</div>';
            item_penjualan += '</td>';

            // total 
            item_penjualan += '<td onclick="Return(' + urutan + ')">';
            item_penjualan += '<div class="form-group">'
            item_penjualan +=
            ...
            item_penjualan += '<button type="button" class="btn btn-primary btn-sm" onclick="MemoEkspedisi(' + urutan +
                ')">';
            item_penjualan += '<i class="fas fa-plus"></i>';
            item_penjualan += '</button>';
            item_penjualan +=
                '<button style="margin-left:10px" type="button" class="btn btn-danger btn-sm" onclick="removePenjualan(' +
                urutan + ')">';
    ...
    <script>
        ...
        function getFaktur(rowIndex) {
            ...
            // var kodeFakturSudahAda = cekKodeFakturSudahAda(kode_penjualan);
        ...
        function cekKodeFakturSudahAda(kodeFaktur) {
            var kodeFakturInputs = $('[id^=kode_penjualan-]').map(function() {
                return $(this).val();
            }).get();
